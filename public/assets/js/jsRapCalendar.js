...
(function ($) {
    $.fn.jsRapCalendar = function (options) {
        return this.each(function () {
            ...
            } else this.caption = false;
            let head = $("<thead>").appendTo(table);
            let dayNames = $("<tr>").appendTo(head);
            for (let n = 0; n < 7; n++)
                $("<th>")
                    .text(this.opt.daysName[(n + 7 - this.opt.week) % 7])
                    .appendTo(dayNames);
            ...
            this.ShowMonth = function (year, month, del) {
                ...
                if (this.caption) {
                    ...
                    if (this.opt.showYear) s += " " + this.curYear;
                    $(this.cap).text(s);
                }
                $(tbody).empty();
                ...
                let td = null;
                let fd =
                    (new Date(this.curYear, this.curMonth, 1).getDay() +
                        this.opt.week) %
                    7;
                for (var n = 0; n < fd; n++) td = $("<td>").appendTo(tr);
                for (var n = 1; n <= dim; n++) {
                    if ($(tr).children().length > 6)
                        tr = $("<tr>").appendTo(tbody);
                    let d = $("<td>").text(n).appendTo(tr);
                    if (
                        D3ToStr(this.curYear, this.curMonth, n) <
                        this.opt.minDate
                    )
                        d.addClass("calDisable");
                    else if (this.opt.enabled) d.addClass("calSelectable");
                    let mnth = this.curMonth + 1;
                    mnth = mnth < 10 ? "0" + mnth : mnth;
                    let nday = n < 10 ? "0" + n : n;
                    let curdate = this.curYear + "-" + mnth + "-" + nday;
                    d.addClass(curdate);
                    if (
                        this.selY == this.curYear &&
                        this.selM == this.curMonth &&
                        ...
                        $(d).addClass("calSelected calcurd");
                }
